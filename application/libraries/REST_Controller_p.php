...
abstract class REST_Controller extends CI_Controller {
    ...
    {
       // $data = 
      // echo "<pre>";print_r($this->input->post());die("come");
        $this->_post_args = $this->input->post();/*array(
    ...
    {   
//echo  $username = $this->input->server('PHP_AUTH_USER');die("come"); 
    // echo "<pre>";print_r(apache_request_headers());die("ccxc");//echo $headers['Authorization'];die("come");
         
         $username= $this->config->item('HTTP_USER');
         $password=$this->config->item('HTTP_PASS');
         $headers = apache_request_headers();
          //echo $username.':'.'REST API'.':'.$password;
         $valid_hash = md5($username.':'.'REST API'.':'.$password);
        
         if(isset($_REQUEST['api_key']))
         {$Authorization2=$_REQUEST['api_key'];}
           else {$Authorization2='';}
          //echo $Authorization2=str_replace("api_key=","",$_SERVER['QUERY_STRING']);die("san");
          $authuser=$this->input->server('PHP_AUTH_USER');
          $authpass=$this->input->server('PHP_AUTH_PW');
        if($authuser!='' && $authpass!='')
        {
           $Authorization= md5($this->input->server('PHP_AUTH_USER').':'.'REST API'.':'.$this->input->server('PHP_AUTH_PW'));
        }
        else {$Authorization='';}
        if($Authorization==$valid_hash || $valid_hash==$Authorization2)
        { 
        return;
        }
